...
void generer_temps_pilote(Pilote *pilote) {
    pilote->secteur_1 = generer_temps_secteur();   // Secteur 1 - très variable
    pilote->secteur_2 = generer_temps_secteur();   // Secteur 2 - très variable
    pilote->secteur_3 = generer_temps_secteur();   // Secteur 3 - très variable
...
bool doit_rester_en_piste() {
    // Logique de décision pour rester en piste ou rentrer aux stands (chance aléatoire)
    int probabilité_rester = rand() % 100; // Probabilité de rester en piste (0 à 99)
    if (probabilité_rester < 90) { // 90% de chance de rester en piste
        return true;
    } else {
        return false; // 10% de chance de rentrer aux stands
    }
}

//fonction qui simule la décision de chaque pilote sur un tour
void gerer_strategie_pilote(Pilote *pilote) {
    if (doit_rester_en_piste()) {
        // Le pilote reste en piste, on génère ses temps de secteurs comme d'habitude
    ...
    for (int tour = 1; tour <= nb_tours; tour++) {
        for (int i = 0; i < nb_pilotes; i++) {
            ...
            if (pid == 0) {
                ...
                // Simuler la stratégie de chaque pilote
                gerer_strategie_pilote(&mp->pilotes[i]);
                
                // Mise à jour des temps du pilote
...
void qualification(MemoirePartagee *mp) {
    ...
    // Phase Q1 - Une génération aléatoire de temps de qualification
    executer_tour(mp, NB_PILOTES, "Q1", NB_TOURS_QUALIF);
    printf("Éliminés après Q1 :\n");
    for (int i = NB_PILOTES / 2; i < NB_PILOTES; i++) printf("%d. Pilote: %s\n", i + 1, mp->pilotes[i].nom);
    ...
    // Phase Q3 - La dernière phase des qualifications
